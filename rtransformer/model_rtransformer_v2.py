...
class TransLayer(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.act = nn.LeakyReLU(1e-1, True)
...
class SubNet(nn.Module):
    ...
    def block_warp(self, x):
        ...
        if self.training and True:
            y = checkpoint_sequential(self.ms, len(self.ms), x, preserve_rng_state=False,use_reentrant = True)
        else:
            y = self.ms(x)
        return y
...
if __name__ == '__main__':
    ...
    net = Net(1024, 128, inter_dim=512, n_block=6, expand_dim=768, squeeze_dim=256, bucket_size=256, bucket_batch_num=256).to(device)
    model_utils_torch.print_params_size(net)
