...
def split_group_v3(s: torch.Tensor, bucket_size):
    assert s.shape[0] == 1
...
class Net(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, inter_dim=512, n_block=6, expand_dim=768, squeeze_dim=256, bucket_size=64, bucket_batch_num=512):
        ...
        self.tr_layer = nn.Sequential(nn.Linear(inter_dim, inter_dim),
                                      nn.LayerNorm(inter_dim),
                                      nn.GELU(),
                                      )
    ...
    def forward(self, xs):
        ...
        for x in xs:
            ...
            while True:
                gys = []
                gxs = split_group_v3(x, self.bucket_size)
                ...
                # [B, L2, C]
                ys.append(self.tr_layer(gys).pow(2).mean(dim=1, keepdim=True))
                # ys.append(self.tr_layer(gys).mean(dim=1, keepdim=True))
                if gys.shape[1] == 1:
                    break
                x = gys
            ys = torch.cat(ys, 1)
            y = ys.pow(2).mean(dim=1)
            # y = ys.mean(dim=1)
            os.append(y)
...
if __name__ == '__main__':
    ...
    net = Net(1024, 128, inter_dim=512, n_block=6, expand_dim=768, squeeze_dim=256, bucket_size=256, bucket_batch_num=512).to(device)
